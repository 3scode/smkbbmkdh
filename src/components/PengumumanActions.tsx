...
export function PengumumanActions({ id, pinned }: { id: string; pinned: boolean }) {
  ...
  async function patch(patch: Record<string, unknown>, kunci: string) {
    ...
    setPesan(null);
    try {
      const res = await fetch(`/api/internal/pengumuman/${id}`, {
        method: "PATCH",
  ...
  async function hapus() {
    if (!confirm("Hapus pengumuman ini permanen? Lanjutkan?")) return;
    setBusy("hapus");
    setPesan(null);
    try {
      const res = await fetch(`/api/internal/pengumuman/${id}`, { method: "DELETE" });
      const json = (await res.json()) as
        { success: true } | { success: false; error: { message: string } };
      if (!json.success) {
        setPesan(json.error.message);
    ...
    <div className="flex flex-col gap-1">
      <div className="flex flex-wrap gap-2">
        <button
          type="button"
          disabled={busy !== null}
          onClick={() => patch({ isPinned: !pinned }, "pin")}
          className={btn}
        >
          {busy === "pin" ? "…" : pinned ? "Lepas pin" : "Pin"}
        </button>
